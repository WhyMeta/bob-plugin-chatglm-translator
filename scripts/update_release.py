...
def update_appcast(version, desc):
    release_file = Path(f'release/chatglm-translator-{version}.bobplugin')
    ...
    version_info = {
        ...
        'desc': desc,
        'sha256': file_hash,
        'url': f'https://github.com/whymeta/bob-plugin-chatglm-translator/releases/download/v{version}/{release_file.name}',
        'minBobVersion': '1.8.0'
    }
    ...
    if appcast_file.is_file():
        ...
        appcast = dict(identifier='whymeta.chatglm.translator', versions=[])
